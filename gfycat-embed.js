...
var gfyCollection = function () {
    ...
    function init() {
		...
		for (var i = 0; i < elem_coll.length; i++) {			
			if (!elem_coll[i].hasAttribute("data-gfycat-init")) {
				elem_coll[i].setAttribute("data-gfycat-init", "true");
				var gfyObj = new gfyObject(elem_coll[i]);
				collection.push(gfyObj);
				added_on_init.push(gfyObj);
			}            
        }
		// this has to be called separately. The init() function modifies the dom, and would mess up our loop if we called init() above.
		for (var x = 0; x < added_on_init.length; x++) {			
			added_on_init[x].init();
		}
		
		return {
			get: get,
			added_on_init: added_on_init			
		}
    }
...
var gfyObject = function (gfyElem) {
    ...
    function setWrapper() {
        gfyRootElem.style.position = "relative";
        gfyRootElem.style.padding = 0;
		gfyRootElem.className = "gfycat-wrapper";
        if (!optExpand) {
    ...
    function init() {
        ...
        if (gfyRootElem.getAttribute('data-autoplay') == "false")
            optAutoplay = false;
        if (gfyRootElem.getAttribute('data-scale-to-fit') == "true") {
            optScaleToFit = true;
			maxWidth 	= gfyRootElem.parentElement.offsetWidth;
			maxHeight 	= gfyRootElem.parentElement.offsetHeight;
		}
        ...
        loadJSONP("https://gfycat.com/cajax/get/" + gfyId, function (data) {
            if (data) {
				...
				gfyRawWidth 	= gfyItem.width;
				gfyRawHeight 	= gfyItem.height;
                ...
                if (document.createElement('video').canPlayType) {
					...
					if (is_ios()) {
						// the events don't fire unless you've clicked directly onto the video.
						// and the video tag doesn't scale up because ios is a cunt. So we just force it to the right size.
						vid.style.width 	= gfyRawWidth + 'px';
						vid.style.height 	= gfyRawHeight + 'px';
						vidLoaded();
					} else { 
    ...
    function setSize() {
		...
		if (is_ios()) {
			gfyWidth 	= gfyRawWidth;
			gfyHeight 	= gfyRawHeight;
		} else {
		...
		if (optScaleToFit) {
			var minRatio 				= Math.min(maxWidth / gfyRawWidth, maxHeight / gfyRawHeight);
			gfyWidth 					= gfyRawWidth * minRatio;
			gfyHeight 					= gfyRawHeight * minRatio;
			// we force wrapper to be this size. The video has a width:100%, height:auto.  So it should adjust to this size.
			gfyRootElem.style.width 	= gfyWidth + 'px';
			gfyRootElem.style.height 	= gfyHeight + 'px';
		}
    ...
    function vidLoaded() {		
		...
		// call the gallery onload so now it will actually display the bastard.
		// we use to call this last.  But now that we are getting the raw image size from the JSON request.  We can call this first.  It 
		// will size it's container div for us.  It will also increase the video size.  Which we can then match for the canvas.
		if (onLoad) {
			onLoad();
		}
	...
	function set_on_load(input_on_load) {	
		onLoad = input_on_load;
	}
	
	function get_raw_size() {
		return { width: gfyRawWidth, height: gfyRawHeight};
	}

    return {
        init: init,
        refresh: refresh,
		set_on_load: set_on_load,
